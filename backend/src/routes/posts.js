...
router.get('/media', auth, async (req, res) => {
  ...
  try {
    ...
    console.error('[posts] GET /media error:', err.message);
...
router.get('/', auth, async (req, res) => {
  ...
  if (group_id) { q += ` WHERE p.group_id=$2 AND (p.archived IS NULL OR p.archived=false)`; params.push(group_id); }
  else { q += ` WHERE p.group_id IS NULL AND (p.archived IS NULL OR p.archived=false)`; }
