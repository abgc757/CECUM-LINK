      ...
      <header className="bg-white border-b border-gray-200 sticky top-0 z-40">
        <div className="max-w-6xl mx-auto px-4 h-14 flex items-center justify-between">
          ...
          <div className="flex items-center gap-1">
            ...
            <button onClick={() => navigate(`/profile/${user.id}`)} className="flex items-center gap-2 p-1 rounded-lg hover:bg-gray-100" title={t('nav.profile')}>
              <Avatar user={user} size="sm" />
              <span className="text-sm font-medium hidden sm:block">{user.full_name.split(' ')[0]}</span>
            </button>
            <button onClick={handleLogout} className="p-2 text-gray-400 hover:text-accent transition-colors" title={t('nav.logout')}>
