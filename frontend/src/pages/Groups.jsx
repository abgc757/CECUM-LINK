  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <button onClick={() => setCreating(v => !v)} className="btn-primary text-sm">+ Crear grupo</button>
      </div>

      {creating && (
        <div className="card p-6">
          <h2 className="font-semibold mb-4">Nuevo grupo</h2>
          <form onSubmit={create} className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="sm:col-span-2">
              <label className="label">Nombre del grupo *</label>
              <input className="input" required value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} placeholder="Ej: Matemáticas 3°B" />
            </div>
            <div>
              <label className="label">Materia</label>
              <input className="input" value={form.subject} onChange={e => setForm(f => ({ ...f, subject: e.target.value }))} placeholder="Matemáticas" />
            </div>
            <div>
              <label className="label">Grado</label>
              <input className="input" value={form.grade} onChange={e => setForm(f => ({ ...f, grade: e.target.value }))} placeholder="3° Secundaria" />
            </div>
            <div className="sm:col-span-2">
        ...
        {groups.map(g => (
          <div key={g.id} className="card p-5 flex flex-col gap-3 hover:shadow-md transition-shadow">
            ...
            <div className="flex items-center justify-between mt-auto">
              <span className="text-xs text-gray-400">{g.members_count} miembros</span>
              <div className="flex gap-2">
                {g.is_member && (
                  <button onClick={() => navigate(`/groups/${g.id}`)} className="btn-secondary text-xs px-3 py-1.5">Ver grupo</button>
                )}
